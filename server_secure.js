...
var peer_server = PeerServer({ port: 9000 ,ssl: server_option});
...
let options = new firefox.Options()/*.addArguments("-headless")*/.setPreference('media.navigator.permission.disabled', true).setProfile(profile);
...
https.createServer(server_option, function (req, res) {
    if (browser == 0) {
        driver.get('https://127.0.0.1:8080/server.html');
        browser = 1;
    }
